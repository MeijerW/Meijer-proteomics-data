        ...
        if gene_input:
            ...
            if rna_avg.empty:
                ...
                g = sns.clustermap(
                    rna_avg,
                    cmap="viridis",
                    figsize=(6, max(6, len(rna_avg) * 0.4)),
                    row_cluster=True,
                    col_cluster=False,
                    cbar_kws={'label': 'Z-score (RNA)'}
                )
                st.pyplot(g.fig)
    
                # --- Protein heatmap aligned to RNA clustering ---
                gene_order = g.data.index.tolist()
                prot_ordered = prot_avg.reindex(gene_order)
    
                fig, ax = plt.subplots(figsize=(6, len(gene_order) * 0.4 + 2))
                sns.heatmap(
                    prot_ordered,
                    cmap="viridis",
                    ax=ax,
                    ...
                    cbar_kws={'label': 'Z-score (Protein)'},
                    yticklabels=True
                )
                ax.set_title("Protein Expression (aligned to RNA clustering)")
                ax.yaxis.tick_right()

        ...
        with subtab2:
            ...
            if gene_input:
                ...
                if rna_avg.empty and prot_avg.empty:
                    ...
                    # Drop rows with any NaNs before clustering
                    cluster_df_clean = cluster_df.dropna(how='any')
                    other_df_clean = other_df.loc[cluster_df_clean.index]
    
                    if cluster_df_clean.empty:
                        st.warning("No complete data available for clustering after removing genes with missing values.")
                    else:
                        if len(cluster_df_clean) == 1:
                            fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(8, 6), sharey=True)
                            vmin = min(np.nanmin(cluster_df_clean.values), np.nanmin(other_df_clean.values))
                            vmax = max(np.nanmax(cluster_df_clean.values), np.nanmax(other_df_clean.values))
                            ...
                            if cluster_on == "RNA":
                                ...
                                ax2.set_yticklabels(other_df_clean.index, rotation=0)
                            else:
                                ax1.yaxis.tick_right()
                                ax1.yaxis.set_label_position("right")
                                ax1.set_yticklabels(other_df_clean.index, rotation=0)
                            ...
                            gene_order = [cluster_df_clean.index[i] for i in g.dendrogram_row.reordered_ind]
                            plt.close()
    
                            cluster_ordered = cluster_df_clean.loc[gene_order]
                            other_ordered = other_df_clean.loc[gene_order]
                            ...
                            # Add colorbars separately
                            norm = plt.Normalize(vmin=vmin, vmax=vmax)
                            sm = plt.cm.ScalarMappable(cmap="viridis", norm=norm)
                            sm.set_array([])
    
                            cbar1 = plt.colorbar(sm, cax=cax1, orientation="horizontal")
                            cbar1.set_label(cluster_label + " Z-score")
                            cbar2 = plt.colorbar(sm, cax=cax2, orientation="horizontal")
                            cbar2.set_label(other_label + " Z-score")
    
                            # Set yticklabels on right heatmap
